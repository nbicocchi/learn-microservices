...
The waterfall model creates friction between (1) requirements gathering and (2) development due to its linear and rigid structure, where each phase must be completed before moving to the next. 
...
- Teams use **automation** to accelerate manual processes. 
...
**Blue-Green Deployment** uses two identical environments—one active (blue) and one idle (green). The new version is deployed to the idle environment and tested. Once verified, all traffic is instantly switched to it.
...
* Requires 2 full environments
* Expensive
* Doesn’t gradually test with real users



**Rolling Deployment** gradually replaces old instances of the application with new ones across the infrastructure. All users are progressively exposed to the new version as the update spreads. 
...
* You don’t need full environment duplication

Strengths

* No duplicate environment needed (Cost efficient)
* Built-in support in most orchestrators

Weaknesses

* Slower rollback
* Old and new versions coexist temporarily
* Risk if backward compatibility is broken

**Canary Deployment** starts by releasing the new version to a small subset of users. If no issues are observed, the rollout continues to a larger user base. 
...
* You have strong **monitoring and observability**
...
* Can combine with feature flags
...
| **Category**                                    | **Tool Examples**           | **Purpose / Use**                                                                                                       |
| ----------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| **Container & Image Scanning**                  | **Trivy**, Clair            | Scan container images for vulnerabilities in OS packages and application dependencies.                                  |
| **Static Code Analysis (SAST)**                 | **Semgrep**, SonarQube      | Detect security vulnerabilities in source code; Semgrep allows **custom pattern-based scans** for security issues.      |
| **Dependency & Open Source Scanning (SCA)**     | **Snyk**, WhiteSource       | Find and fix vulnerabilities in third-party libraries.                                                                  |
| **Secrets & Credential Scanning**               | **GitGuardian**, TruffleHog | Detect exposed secrets in code or repositories; GitGuardian specializes in **real-time secret detection in Git repos**. |
| **Code Linting / Quality**                      | **Super-Linter**, ESLint    | Enforce code quality and basic security rules across multiple languages.                                                |
| **Dynamic Application Security Testing (DAST)** | **OWASP ZAP**, Burp Suite   | Test running applications for runtime vulnerabilities.                                                                  |
| **Infrastructure as Code (IaC) Security**       | **Checkov**, tfsec          | Analyze Terraform, CloudFormation, or Kubernetes manifests for misconfigurations.                                       |
| **Runtime / Container Security**                | **Falco**, Aqua Security    | Monitor live containers and cloud workloads for suspicious activity.                                                    |
