...
- Useful in steady-state systems with predictable traffic patterns.

**Cons:**
- **Boundary Issues**: Failures at the end of one window are forgotten when a new window starts, which can lead to inaccurate failure detection. For example, a burst of errors at the end of a window may not open the circuit if a new window starts shortly after.
- **Delayed Response**: Since failures are only evaluated at the end of the window, the circuit breaker may take longer to respond to a spike in failures.
...
**Pros:**
- **More Accurate**: Provides continuous, real-time error tracking, which allows the circuit breaker to react faster to changes.
- **Smoother Detection**: There are no sharp resets, so failures are consistently tracked.

**Cons:**
- **Higher Complexity**: Sliding windows are more complex to implement since they require continuous tracking of requests and failures over multiple sub-windows.
- **Performance Overhead**: Updating the sliding window in real time requires additional computations.
...
- **Smooths Bursts**: Unlike fixed windows, which may miss brief bursts of failures, the leaky bucket smooths out fluctuations in failure rates.
- **Prevents Overload**: It helps protect systems from being overwhelmed by sudden spikes in traffic by gradually letting failures "leak."

**Cons:**

- **Not Real-Time**: The leaky bucket doesn't react as fast as a sliding window because it's designed to smooth out bursts of failures over time, not to instantly react.
- **Configuration Complexity**: Tuning the bucket size and leak rate can be challenging for finding the right balance between system resilience and responsiveness.

#### Comparison of Fixed Window, Sliding Window, and Leaky Bucket
