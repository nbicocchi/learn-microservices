# DevOps, DevSecOps & Pre-commit hooks (GitLab CI, GitGuardian, SemGrep, Trivy)
...
Enable the **Compute Engine API** on your project.

Crea a new **VM instance**.
...
Insert the pub key into **authorized_keys** file:
...
services:
  product:
    image: registry.gitlab.com/michelemosca/devsecops-pipeline-development
    mem_limit: 512m
    environment:
...
We can test it using the **test-em-all.bash** script:
