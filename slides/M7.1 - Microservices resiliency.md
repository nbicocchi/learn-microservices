...
* **Time-sensitive operations**: Retries may not be appropriate for time-critical operations. The trade-off here is to decrease a timeout and enable retries or keep the max acceptable timeout value. Retries might not work well where 99th percentile is close to 50th percentile. Look at the graphs below. On the first one, timeouts occasionally happens, a good case for enabling retries. On the second graph, timeouts happen periodically, do not enable retries.
...
TODO: How to measure a service latency?
...
![](images/microservices-resiliency-bulkhead-1.png)
...
![](images/microservices-resiliency-bulkhead-2.png)
