...
* **Customers want faster delivery**: Customers no longer want to wait for the next annual release of a software package. Instead, they expect the features in a soft- ware product to be unbundled so that new functionality can be released quickly in a matter of weeks (or even days).
