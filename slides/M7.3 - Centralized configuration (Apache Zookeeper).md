...
zookeeper-config-service code is composed by the following steps:
...
In the directory of zookeeper-config-service we find the code of zookeeper-config-service.

Now we can run zookeeper-config-service to create and populate the Zookeeper /config node:
