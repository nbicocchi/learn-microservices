...
With the thread per request programming model, we have a dispatcher component creating a new thread object for managing the steps of each request. Everything looks nice, but **there is a catch: context switching and memory consumption!**
...
So the event-driven programming model does not increase performance, but allows to process concurrent requests using less RAM. This is key in global-scale applications, facing hundreds or thousands of requests per second.
