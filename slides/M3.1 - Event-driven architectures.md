...
**Messaging is inherently asynchronous, so only provides asynchronous request/response**. When a client and service interact using either request/response or asynchronous request/response, the client sends a request and the service sends back a reply. 

The difference between the two interaction styles is that:
* with request/response the client expects the service to respond immediately
* with asynchronous request/response there is no such expectation: the client must tell the service where to send a reply message and must match reply messages to requests.
